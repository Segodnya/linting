// Example violating ESLint rules
...
// Error:  no-sequences
do { console.log('no-sequences'); } while (console.log('do smth'), !!test);
...
let returnfoo = function() { return; };

// Error: radix
let radixSecondArgument = parseInt('071', 10);

// Error: wrap-iife
let wrapped = function () { return { y: 1 };}();
...
// Error: block-spacing
function blockSpacing() {return true;}
...
function foo() {
    for (;;) { // Nested 1 deep
        while (true) { // Nested 2 deep
            if (bar) { // Nested 3 deep
                if (bar) { // Nested 4 deep
                    if (bar) { // Nested 5 deep
                    }
                }
            }
        }
